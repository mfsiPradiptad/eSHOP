...
class ProductRepository
{
    ...
    {
        $strProduct = $searchFilters['productName'];
        $selectCat = $searchFilters['selectCat'];
        $selectFor = $searchFilters['selectFor'];

        $rows = Product::query();

        if ($strProduct != '') {
            $rows->where('productName', 'like', '%' . $strProduct . '%');
        }

        if($selectCat > 0) {
            $rows->where('intCategory', $selectCat);
        }

        if($selectFor > 0) {
            $rows->where('intFor', $selectFor);
        }
    ...
    public function fillAnxure(int $intAnxId): array
    {
        $rows = Annexures::where('intAnxCat', $intAnxId)
                         ->get();
